    ...
    namespace app {
        namespace client {
            ...
            bool VEthernetNetworkSwitcher::OnUdpPacketInput(const std::shared_ptr<IPFrame>& packet) noexcept {
                ...
                if (block_quic_ && destinationPort == 443) {
                    return false;
                }
                ...
                if (static_mode_) {
                    ...
                    if (static_.quic && destinationPort == 443) {
            ...
            void VEthernetNetworkSwitcher::ReleaseAllTimeouts() noexcept {
                ...
                Dictionary::ReleaseAllCallbacks(timeouts);
            }
            ...
            static ppp::function<ppp::string(ppp::net::native::RouteEntry&)> Linux_GetNetworkInterfaceName(
                ...
                auto f = [tap_if, tap_ni, underlying_ni](ppp::net::native::RouteEntry& entry) noexcept {
            ...
            bool VEthernetNetworkSwitcher::Open(const std::shared_ptr<ITap>& tap) noexcept {
                ...
                if (configuration_->udp.static_.aggligator > 0) {
...
#elif defined(_MACOS)
                    ppp::darwin::tun::utun_add_route2(IPEndPoint::AnyAddress, IPEndPoint::AnyAddress, dw);
#else
            ...
            bool VEthernetNetworkSwitcher::LoadAllDnsRules(const ppp::string& rules, bool load_file_or_string) noexcept {
                ...
                if (load_file_or_string) {
                    events = ppp::app::client::dns::Rule::LoadFile(rules, dns_rules_);
                }
                else {
                    events = ppp::app::client::dns::Rule::Load(rules, dns_rules_);
                }
            ...
            bool VEthernetNetworkSwitcher::AddRemoteEndPointToIPList(const boost::asio::ip::address& gw) noexcept {
                ...
                if (std::shared_ptr<aggligator::aggligator> aggligator = aggligator_; NULL != aggligator) {
                    if (!aggligator->client_open(configuration_->udp.static_.aggligator, servers)) {
                        return false;
                    }
                }
            ...
            bool VEthernetNetworkSwitcher::RedirectDnsServer(
                ...
                boost::system::error_code ec;
                boost::asio::ip::address serverIP = rulePtr->Server;
                ...
                socket->async_receive_from(boost::asio::buffer(buffer.get(), max_buffer_size),
                    *reinterpret_cast<boost::asio::ip::udp::endpoint*>(buffer.get() + max_buffer_size),
                    [self, this, socket, timeout, buffer, rulePtr, sourceEP, destinationEP](boost::system::error_code ec, size_t sz) noexcept {
            ...
            bool VEthernetNetworkSwitcher::RedirectDnsServer(const std::shared_ptr<VEthernetExchanger>& exchanger, const std::shared_ptr<IPFrame>& packet, const std::shared_ptr<UdpFrame>& frame, const std::shared_ptr<BufferSegment>& messages) noexcept {
                ppp::string hostDomain = ppp::net::native::dns::ExtractHost(messages->Buffer.get(), messages->Length);
                if (hostDomain.empty()) {
                    return false;
                }

                ppp::app::client::dns::Rule::Ptr rulePtr = ppp::app::client::dns::Rule::Get(hostDomain, dns_rules_);
                if (NULL == rulePtr) {
                    return false;
                }
                ...
                return ppp::coroutines::YieldContext::Spawn(allocator.get(), *context,
                    [self, this, socket, buffer, rulePtr, frame, messages, context, destinationIP](ppp::coroutines::YieldContext& y) noexcept {
                        return RedirectDnsServer(y, socket, buffer, rulePtr, frame, messages, context, destinationIP);
                    });
            }

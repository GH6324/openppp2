...
#define PPP_APPLICATION_VERSION ("1.0.0.24191") /* 1.0.0.240506 */
#endif
...
namespace ppp {
    ...
    std::shared_ptr<void*>                                                  make_shared_void_pointer(A&&... args) noexcept {
        ...
        void* memory = (void*)Malloc(sizeof(T));
        if (NULL == memory) {
            return NULL;
        }

        T* m = new (memory) T(std::forward<A&&>(args)...);
        return std::shared_ptr<void*>(reinterpret_cast<void**>(m),
            [](void** p) noexcept {
